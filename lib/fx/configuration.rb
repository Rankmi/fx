module Fx
  ...
  class Configuration
    ...
    #
    # Defaults to a lambda which allows all triggers - by ScottLangridge
    # @return Proc<Boolean>
    attr_accessor :include_trigger_from_schema_condition

    # A lambda returning a boolean to indicate whether or not a given function
    # should be allowed to be dumped into schema.rb
    #
    # Defaults to a lambda which allows all function - by ScottLangridge
    # @return Proc<Boolean>
    attr_accessor :include_function_from_schema_condition

    def initialize
      @database = Fx::Adapters::Postgres.new
      @dump_functions_at_beginning_of_schema = false
      @include_trigger_from_schema_condition = lambda { |trigger| false }
      @include_function_from_schema_condition = lambda { |function| false }
    end
  end
end
