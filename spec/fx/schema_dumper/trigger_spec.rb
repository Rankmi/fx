...
describe Fx::SchemaDumper::Trigger, :db do
  ...
  it "dumps only included triggers" do
      begin
          Fx.configuration.include_trigger_from_schema_condition = lambda do |trigger|
      ...
      CREATE TRIGGER lowercase_users_name
          ...
          output = stream.string
          expect(output).to include "create_trigger :lowercase_users_name"
          expect(output).not_to include "create_trigger :uppercase_users_name"
          expect(output).to include "sql_definition: <<-SQL"
